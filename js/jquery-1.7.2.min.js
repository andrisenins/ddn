 ...
 * 
 * Copyright 2005, 2013 jQuery Foundation, Inc. and other contributors Released under the MIT license
...
(function(window, undefined) {
        ...
        parseJSON: function(data) {
          ...
          if (typeof data === "string") {
            ...
            if (data) {
              ...
              if (rvalidchars
                  .test(data.replace(rvalidescape, "@").replace(rvalidtokens, "]").replace(rvalidbraces, ""))) {
  ...
  (function(window, undefined) {
    ...
    setDocument = Sizzle.setDocument = function(node) {
      ...
      sortOrder = docElem.compareDocumentPosition ? function(a, b) {
        ...
        if (a === b) {
          ...
          return a === doc ? -1 : b === doc ? 1 : aup ? -1 : bup ? 1 : sortInput ? (indexOf.call(sortInput, a) - indexOf
              .call(sortInput, b)) : 0;
    ...
    Sizzle.attr = function(elem, name) {
      ...
      return val === undefined ? support.attributes || !documentIsHTML ? elem.getAttribute(name) : (val = elem
        ...
        "ATTR": function(name, operator, check) {
          return function(elem) {
            ...
            return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check &&
                ...
                result.slice(-check.length) === check : operator === "~=" ? (" " + result + " ").indexOf(check) > -1 : operator === "|=" ? result === check ||
    ...
    if (!assert(function(div) {
      ...
      addHandle(booleans, function(elem, name, isXML) {
        var val;
        if (!isXML) {
          return (val = elem.getAttributeNode(name)) && val.specified ? val.value : elem[name] === true ? name
              .toLowerCase() : null;
        }
      });
    }
          ...
          select: {
            get: function(elem) {
              var value, option, options = elem.options, index = elem.selectedIndex, one = elem.type === "select-one" ||
                  index < 0, values = one ? null : [], max = one ? index + 1 : options.length, i = index < 0 ? max : one ? index : 0;
  ...
  jQuery.event = {
    ...
    trigger: function(event, data, elem, onlyHandlers) {
      var handle, ontype, cur, bubbleType, special, tmp, i, eventPath = [elem || document], type = core_hasOwn.call(
          event, "type") ? event.type : event, namespaces = core_hasOwn.call(event, "namespace") ? event.namespace
          .split(".") : [];
          ...
          if (cur.nodeType === 1 && (cur.disabled !== true || event.type !== "click")) {
            ...
            for (i = 0; i < delegateCount; i++) {
              ...
              if (matches[sel] === undefined) {
                matches[sel] = handleObj.needsContext ? jQuery(sel, this).index(cur) >= 0 : jQuery.find(sel, this,
                    null, [cur]).length;
              }
              if (matches[sel]) {
  ...
  jQuery
      .extend({
        filter: function(expr, elems, not) {
          ...
          return elems.length === 1 && elem.nodeType === 1 ? jQuery.find.matchesSelector(elem, expr) ? [elem] : [] : jQuery.find
  ...
  if (!jQuery.support.opacity) {
    jQuery.cssHooks.opacity = {
      get: function(elem, computed) {
        ...
        return ropacity.test((computed && elem.currentStyle ? elem.currentStyle.filter : elem.style.filter) || "") ? (0.01 * parseFloat(RegExp.$1)) +
            "" : computed ? "1" : "";
      },

      set: function(elem, value) {
        var style = elem.style, currentStyle = elem.currentStyle, opacity = jQuery.isNumeric(value) ? "alpha(opacity=" +
            value * 100 + ")" : "", filter = currentStyle && currentStyle.filter || style.filter || "";
  ...
  /*
   * Prefilters 1) They are useful to introduce custom dataTypes (see ajax/jsonp.js for an example) 2) These are called: -
   * BEFORE asking for a transport - AFTER param serialization (s.data is a string if s.processData is true) 3) key is
   * the dataType 4) the catchall symbol "*" can be used 5) execution will start with transport dataType and THEN
  ...
  /*
   * Chain conversions given the request and the original response Also sets the responseXXX fields on the jqXHR
   * instance
   */
